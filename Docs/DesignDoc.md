...
| `LockOnAimPointNode` | Builds a stable virtual aim point for lock-on cameras. Follow and Aim each resolve through a `Source` pair (`WorldPosition` or `ActorPosition` + actor source / world-up offset), then the node applies the pitch-preserving / camera-to-aim / camera-forward offset blend used by the legacy `ModifyAimPointExtension` and outputs `PivotPosition` for a downstream `ScreenSpacePivot` instead of mutating the target actor. While inside `Radius`, the pitch-preserving term uses the current follow-to-aim pitch when it falls inside `PitchRange`, or the nearest `PitchRange` boundary when it falls outside that range, so a falling target does not keep its entry pitch forever. When leaving `Radius`, the combined correction offset fades out over `BlendOutTime` instead of snapping back to the raw actor position. |
...
- **Shipped node overrides** (each with its own CVar, default 0):
    ...
    - `LockOnAimPointNode` (`CCS.Debug.Viewport.LockOnAimPoint`) - blue sphere at the stable virtual aim point. During correction and F8 eject, draws a line from the raw aim point to the corrected aim point.
