 ...
 * tick outputs `InitialFOV` / `InitialDistance` exactly, with no seam at t=0.
 *
...
public:
	...
	/** Selects whether the lock subject is the controller's controlled pawn
	 *  or the explicitly supplied PivotActor. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = InputParameters)
	EComposableCameraActorInputSource PivotActorSource { EComposableCameraActorInputSource::ExplicitActor };

	/** The explicit subject the effect locks on. Camera dollies along the
	 *  camera-to-subject axis; FOV compensates so this subject stays the
	 *  same on-screen size when PivotActorSource uses ExplicitActor. */
	 ...
	 *  carried for FieldOfView. This is the typical case for camera type assets
	 ...
	 *  chain, preserving the previous behavior.
	 *
	...
	/** Additive FOV delta (degrees) over normalized time. X is in [0, 1],
	...
	/** Additive distance delta (world units) over normalized time. X is in
