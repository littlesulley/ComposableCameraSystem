 ...
 *
 * ~48 bytes per entry × 120-entry capacity = ~6 KB of ring memory per
 * PCM. Negligible.
 */
struct FComposableCameraPoseHistoryEntry
{
	 ...
	 *  Pauses with the game so the timeline doesn't drift while paused —
	 *  that's the semantic users expect when scrubbing history. */
	float GameTime = 0.f;
