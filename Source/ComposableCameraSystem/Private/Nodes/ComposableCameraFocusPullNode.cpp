...
bool UComposableCameraFocusPullNode::ResolveTargetPoint(FVector& OutTargetPoint) const
{
	...
	{
		// First skeletal mesh with the requested bone wins. Same as
		// OcclusionFadeNode's resolve path so authors can use the same
		// BoneName wiring across both nodes on the same actor.
		...
		// Bone not found — fall through to actor + Z offset.
	}
