...
namespace
{
	...
	static void AppendTemplatesSection(
	...
	static void AppendPinConnections(
		FStringBuilderBase& B,
		const TCHAR* Label,
	...
	static void AppendExecutionOrder(
		FStringBuilderBase& B,
		const TCHAR* Label,
	...
	static void AppendFullExecChain(
		FStringBuilderBase& B,
		const TCHAR* Label,
		const TArray<FComposableCameraExecEntry>& Chain)
	{
		...
		for (int32 i = 0; i < Chain.Num(); ++i)
		{
			...
			case EComposableCameraExecEntryType::SetVariable:
				B.Appendf(
					TEXT("    [%d] SetVariable guid=%s name='%s' sourcePin='%s' size=%d (from node %d)\n"),
					i,
	...
	static void AppendExposedParameters(
		FStringBuilderBase& B,
		const TArray<FComposableCameraExposedParameter>& Params)
	{
		...
		for (int32 i = 0; i < Params.Num(); ++i)
		{
			...
			B.Appendf(
				TEXT("    [%d] '%s' type=%d target=%d.%s required=%s\n"),
				i,
	...
	static void AppendInternalVariables(
		FStringBuilderBase& B,
		const TCHAR* Label,
		const TArray<FComposableCameraInternalVariable>& Vars)
	{
		...
		for (int32 i = 0; i < Vars.Num(); ++i)
		{
			...
			B.Appendf(
				TEXT("    [%d] '%s' guid=%s type=%d init='%s' resetEveryFrame=%s\n"),
				i,
	...
	static void AppendVariableNodeRecords(
		FStringBuilderBase& B,
		const TArray<FComposableCameraVariableNodeRecord>& Records)
	{
		...
		for (int32 i = 0; i < Records.Num(); ++i)
		{
			...
			B.Appendf(
				TEXT("    [%d] node=%s var='%s' (guid=%s) setter=%s computeChain=%s pos="),
				i,
	...
	static FAutoConsoleCommandWithWorldAndArgs GCmdDumpGraph(
		TEXT("CCS.Editor.Dump.Graph"),
