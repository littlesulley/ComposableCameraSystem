...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FShotEditorComputeWheelZoomFactorTest,
...
bool FShotEditorComputeWheelZoomFactorTest::RunTest(const FString& /*Parameters*/)
{
	...
	// Both held — falls back to default (refuses to compose 5× and 0.2×).
	...
	TestEqual(TEXT("Custom DefaultStep=0.05 + Shift → 1.25 (5× step)"),
		CustomShift, 1.25f);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FShotPlacementDistanceClampInvariantsTest,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FShotEditorReverseSolveStatusToTextTest,
...
bool FShotEditorReverseSolveStatusToTextTest::RunTest(const FString& /*Parameters*/)
{
	...
	for (EShotEditorReverseSolveStatus Status : FailureCases)
	{
		...
		TestFalse(
			FString::Printf(TEXT("Status %d returns non-empty FText"),
		...
		TestFalse(
			FString::Printf(TEXT("Status %d text is unique among failure cases"),
				static_cast<int32>(Status)),
